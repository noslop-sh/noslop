...
fn test_format_trailers_single() {
    ...
    let trailers = store.format_trailers(&[ack]);
    assert_eq!(trailers, "Noslop-Ack: CHK-1 | Reviewed the code | human");
}
...
fn test_format_trailers_multiple() {
    let store = TrailerAckStore::new();
    let acks = vec![
        Acknowledgment::new("CHK-1".to_string(), "First review".to_string(), "human".to_string()),
        Acknowledgment::new(
            "CHK-2".to_string(),
    ...
    let trailers = store.format_trailers(&acks);
    let lines: Vec<&str> = trailers.lines().collect();

    assert_eq!(lines.len(), 2);
    assert_eq!(lines[0], "Noslop-Ack: CHK-1 | First review | human");
    assert_eq!(lines[1], "Noslop-Ack: CHK-2 | Second review | claude-3-opus");
}
...
fn test_format_trailers_escapes_pipe() {
    ...
    assert_eq!(trailers, "Noslop-Ack: CHK-1 | Message with - pipe character | human");
}
...
fn test_append_trailers_to_simple_message() {
    ...
    let trailers = "Noslop-Ack: CHK-1 | Reviewed | human";

    let result = append_trailers(message, trailers);
    assert_eq!(result, "Add new feature\n\nNoslop-Ack: CHK-1 | Reviewed | human");
}
...
fn test_append_trailers_message_with_existing_trailers() {
    ...
    assert!(result.contains("Noslop-Ack:"));
}
...
fn test_append_trailers_multiline_message() {
    ...
    assert!(result.ends_with("Noslop-Ack: CHK-1 | Reviewed | human"));
}

#[test]
fn test_append_multiple_trailers() {
    let message = "Add new feature";
    let trailers = "Noslop-Ack: CHK-1 | First | human\nNoslop-Ack: CHK-2 | Second | agent";

    let result = append_trailers(message, trailers);
    assert!(result.contains("CHK-1"));
    assert!(result.contains("CHK-2"));
}
